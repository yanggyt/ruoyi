...
function windowOpen(url, name, width, height){
	var top1 = parseInt((window.screen.height-height)/2,10),
		left = parseInt((window.screen.width-width)/2,10),
		options = "location=no,menubar=no,toolbar=no,dependent=yes,minimizable=no,modal=yes,alwaysRaised=yes,"+
		"resizable=yes,scrollbars=yes,"+"width="+width+",height="+height+",top="+top1+",left="+left;
...
//跳转页面
function getToPage(){
	inputPageValue = $("#inputPage").val();
	var regexp=/^\d+$/;  
    if(!regexp.test(inputPageValue)||inputPageValue==''){  
        alert("请输入 正确的数字！");  
        return false;  
    }else{  
        return inputPageValue;  
    } 
}

function goToPage(toPage,pageSize,funcParams){
	page(toPage,pageSize,funcParams);
}
